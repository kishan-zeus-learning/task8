...
export class RowsCanvas {
    /** @type {RowData} Map holding the heights of individual rows */
    readonly rowHeights: RowData;

    /** @type {number[]} Array storing cumulative heights for quick lookup of row boundaries */
    readonly rowsPositionArr: number[];

    /** @type {number} Identifier for this block of 25 rows */
    readonly rowID: number;

    /** @type {HTMLDivElement} Container div element holding the canvas and resize handle */
    ...
    /** @type {number} Default width for this canvas block */
    private defaultWidth: number;

    /** @type {number} Default height for each row */
    ...
    /** @type {MultipleSelectionCoordinates} Current selection coordinates within the spreadsheet */
     ...
     */
    drawCanvas(): void {
        if (!this.rowCanvas) return; // Exit if canvas element is not available
        ...
        if (this.ifSelectedWhole()) {
            ...
            if (
                canvasEndRow <= this.rowID * 25 + 25 && // Selection ends within or before this block
            ...
            if (
                canvasStartRow <= this.rowID * 25 + 25 && // Selection starts within or before this block
